                    ...
                    <div class="card shadow-sm border-0 p-4 mt-30">
                      ...
                      <form id="location_preferences_form" method="POST" onsubmit="return update_location_preferences()">
                        ...
                        <!-- <div class="form-check  mt-1  mb-2">
                          ...
                          <label class="toggle-label">Open to international relocation/discovery</label>
                          <label class="switch">
                        ...
                        <div class="toggle-wrapper">
                            <label for="toggleBtn" class="toggle-label">
                                Open to international relocation/discovery
                            </label>

                            <label class="switch">
                                <input type="checkbox" name="international_toggle" id="toggleBtn" @if(!empty($work_preferences_data) && $work_preferences_data->international_relocation == 1)  checked @endif>
                                <span class="slider"></span>
                            </label>
                        </div>
                        ...
                        <div class="location_finder internation_relocation" @if(!empty($work_preferences_data) && $work_preferences_data->international_relocation != 1)  style="display:none" @endif>
                          <h6 class="emergency_text">
                          ...
                          <div class="work_preferences_text">
                            <p>
                                Your profile may appear in Find Nurse searches in the selected countries. 

        
                            </p>
                            <p>
                                Employers can view your profile and invite you to interview. 
                                If you are not registered in that country, your profile will appear as “Requires Sponsorship.” 
                                To browse and apply for jobs directly, add the country in Registrations & Licences and switch your Active Search Country.
                          ...
                          <div class="form-group level-drp">
                            ...
                            <?php
                              if (!empty($work_preferences_data)) {
                                  $countries_data_work = json_decode($work_preferences_data->countries, true) ?? [];
                                  $deleted_country     = json_decode($work_preferences_data->deleted_countries, true) ?? [];
                              } else {
                                  $countries_data_work = [];
                                  $deleted_country     = [];
                              }
                              ...
                              $registered_countries = DB::table("users")->where("id", $user_id)->first();

                              $reg_country = json_decode($registered_countries->registration_countries, true) ?? [];

                              // ✅ Step 1: Merge registered + target
                              $country_target = array_unique(array_merge($countries_data_work, $reg_country));

                              // ✅ Step 2: Remove deleted suggested countries
                              $country = array_values(array_diff($country_target, $deleted_country));

                              // Final JSON for frontend
                              $country_json = json_encode($country);
                            ?>
                            <input type="hidden" name="deleted_country" class="deleted_country" value='@json($deleted_country)'>
                            <input type="hidden" name="country_data" class="country_data" value='<?= $country_json ?>'>
                            <ul id="countries_data" style="display:none;">
                              @php $country_data=country_name_from_db();@endphp
                              @if(!empty($country_data))
                              @foreach($country_data as $cdata)
                              <li data-value="{{ $cdata->iso2 }}">{{ $cdata->name }}</li>
                              @endforeach
                          ...
                          @php
                              $selectedCountries = json_decode($work_preferences_data->countries ?? '[]', true);
                              $registeredCountries = json_decode($registered_countries->registration_countries ?? '[]', true);

                              $workRights = json_decode($work_preferences_data->work_rights ?? '{}', true);
                              $pathway = json_decode($work_preferences_data->pathway ?? '{}', true);
                          @endphp
                          <div id="countryQuestionsWrapper">

                          @if(!empty($selectedCountries))
                          @foreach($selectedCountries as $country)

                              @php
                                  $isRegistered = in_array($country, $registeredCountries);
                                  $selectedWorkRight = $workRights[$country] ?? '';
                                  $selectedPathway = $pathway[$country] ?? '';
                              @endphp

                              <div class="country-block country-block-{{ $country }} mb-3 p-3 border rounded">
                                  ...
                                  <div>
                                      ...
                                      <p class="{{ $isRegistered ? 'text-success' : 'text-warning' }}">
                                          {{ $isRegistered ? 'Locally registered' : 'Not yet registered' }}

                                          @if(!$isRegistered)
                                              <span title="If you are already registered, please update your profile under Registrations & Licences">
                                                  ℹ
                                              </span>
                                          @endif
                                      </p>
                                  ...
                                  <div class="mb-2">
                                      ...
                                      <select name="work_rights[{{ $country }}]" class="form-control">
                                          <option value="">Select</option>
                                          <option value="have" {{ $selectedWorkRight == 'have' ? 'selected' : '' }}>
                                              I have work rights
                                          </option>
                                          <option value="visa" {{ $selectedWorkRight == 'visa' ? 'selected' : '' }}>
                                  ...
                                  @if(!$isRegistered)
                                  <div>
                                      <label>Registration Pathway</label>
                                      <select name="pathway[{{ $country }}]" class="form-control">
                                          <option value="">Select</option>
                                          <option value="bridging" {{ $selectedPathway == 'bridging' ? 'selected' : '' }}>
                                              Bridging program
                                          </option>
                                          <option value="supervised" {{ $selectedPathway == 'supervised' ? 'selected' : '' }}>
                                              Supervised practice
                                          </option>
                                          <option value="not_sure" {{ $selectedPathway == 'not_sure' ? 'selected' : '' }}>
                                              Not sure yet
                                          </option>
                                      </select>
                                  </div>
...
<script id="p9e3sf">
    let registeredCountries = @json($reg_country); // ["US","UK"]
</script>
<script>
  $("#toggleBtn").change(function(){
    if($(this).is(':checked')){
      $(".internation_relocation").show();
    }else{
      $(".internation_relocation").hide();
    }
  });
</script>
<script>
...
<script type="text/javascript">
    ...
    let original = JSON.parse($(".country_data").val() || "[]");
    let alreadyDeleted = JSON.parse($(".deleted_country").val() || "[]");

    // ✅ Find removed values (from original list)
    let removedNow = original.filter(val => !selectedValues.includes(val));

    // ✅ Only keep those which belong to registered (important logic)
    let finalDeleted = [...new Set([...alreadyDeleted, ...removedNow])];

    $(".deleted_country").val(JSON.stringify(finalDeleted));

    console.log("Selected:", selectedValues);
    console.log("Removed Now:", removedNow);
    console.log("Final Deleted:", finalDeleted);

    renderCountryQuestions(selectedValues);

    // ✅ Other toggle logic
    if (selectedValues.includes("Other")) {
        $(".countries_other_div").removeClass("d-none");
    } else {
        $('.js-example-basic-multiple[data-list-id="countries_data_other"]')
            .val([]).trigger('change');

        $(".countries_other_div").addClass("d-none");
    }
});
function renderCountryQuestions(countries) {

    let container = $("#countryQuestionsWrapper");
    //container.empty();

    countries.forEach(country => {
        console.log("country",$(".country-block-"+country).length);
        let isRegistered = registeredCountries.includes(country);
        
        if($(".country-block-"+country).length <= 0){
          let html = `
              <div class="country-block mb-3 p-2 border">
                  <h5>${country}</h5>
                  ...
                  <div>
                      ...
                      <p class="${isRegistered ? 'text-success' : 'text-warning'}">
                          ...
                          ${!isRegistered ? `
                              <span title="If you are already registered, please add it in your profile under Registrations & Licences">
                                  ℹ
                              </span>
                          ` : ''}
                      </p>
                  ...
                  <div>
                      ...
                      <select name="work_rights[${country}]" class="form-control">
                          <option value="">Select</option>
                          <option value="have">I have work rights</option>
                          <option value="visa">I require visa sponsorship</option>
                      </select>
                  </div>
                  ...
                  <div>
                      ...
                      <select name="pathway[${country}]" class="form-control">
                          <option value="">Select</option>
                          <option value="bridging">Bridging program</option>
                          <option value="supervised">Supervised practice</option>
