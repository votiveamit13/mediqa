...
`);
    ...
    function add_work_experience() {
     ...
     function getCertData(){
        let selectedValues = $('.js-example-basic-multiple[data-list-id="profess_cert"]').val();
        console.log("selectedValues",selectedValues);
        ...
        for(var i=0;i<selectedValues.length;i++){
            if($(".subeducational_trainingdata .courses_div-"+selectedValues[i]).length < 1 && selectedValues[i] != 30){
                //$("#submitProfessionalMembership").attr("disabled", true);
                $.ajax({
                    type: "GET",
                    url: "{{ url('/nurse/getEducationCourses') }}",
                    ...
                    success: function(data){
                        ...
                        for(var j=0;j<data1.getCourses.length;j++){
                            ...
                            courses_text += "<li data-value='"+data1.getCourses[j].professionalcert_id+"'>"+data1.getCourses[j].name+"</li>"; 

                        }

                        var ap = '';
                        $(".subeducational_trainingdata").append('\<div class="courses_div courses_div-'+data1.courses_id+'"><div class="form-group level-drp mandatory_courses_div  mandatory_tr_div_1">\
                            ...
                            <select class="js-example-basic-multiple'+data1.courses_id+' addAll_removeAll_btn" data-list-id="well_self_care_data-'+data1.courses_id+'" name="well_self_care_data[]" onchange="showEducationData(\''+ap+'\',\''+data1.courses_id+'\',\''+data1.courses_name+'\')" multiple="multiple"></select>\
    ...
    function showEducationData(ap,id,name){
        if(ap == 'ap'){
        ...
        for(var i=0;i<selectedValues.length;i++){
            console.log("wel_self_care",".well_self_care_div-"+id+" .well_self_care_"+selectedValues[i]);
            if($(".well_self_care_div-"+id+" .well_self_care_"+selectedValues[i]).length < 1){
                $.ajax({
                    type: "GET",
                    url: "{{ url('/nurse/getEducationCoursesName') }}",
                    data: {courses_id:selectedValues[i],id:i},
                    cache: false,
                    success: function(data){
                        var data1 = JSON.parse(data);
                        ...
                        var well_sal_data = "general_certification";
                        $(".well_self_care_div-"+id).append('<div class="well_self_care_'+id+' well_self_care_'+data1.courses_id+'">\
                            ...
                            <div class="row">\
                                ...
                                <input class="form-control upload_evidence-'+data1.courses_id+' well_upload_certification well_imgs_'+selectedValues[i]+' well_upload_certification-'+data1.courses_id+'" type="file" name="" onchange="changeEducationImg(\''+user_id+'\',\''+id+'\',\''+data1.courses_id+'\',\''+well_sal_data+'\')"  multiple>\
    ...
    let selectedEduEvidenceFiles = [];

    function changeEducationImg(user_id,lang_id,language_id,name_arr) {

      if (!selectedEduEvidenceFiles) {
        selectedEduEvidenceFiles = [];
      }
      
      const newFiles = Array.from($('.upload_evidence-'+language_id)[0].files);

      newFiles.forEach(file => {
        const exists = selectedEduEvidenceFiles.some(f => f.name === file.name && f.lastModified === file.lastModified);
        if (!exists) {
            selectedEduEvidenceFiles.push(file);
        }
      });

      console.log("selectedEduEvidenceFiles",selectedEduEvidenceFiles);

      

        const count = selectedEduEvidenceFiles.length;
        console.log("evidence_count", count);
    
      var form_data = "";
      form_data = new FormData();

      for (var i = 0; i < selectedEduEvidenceFiles.length; i++) {
        form_data.append(name_arr+"["+language_id+"][]", selectedEduEvidenceFiles[i], selectedEduEvidenceFiles[i]['name']);
      }
      ...
      $.ajax({
        type: "post",
        url: "{{ route('nurse.uploadEduEvidenceImgs') }}",
        cache: false,
        contentType: false,
        processData: false,
        async: true,
        ...
        success: function(data) {
          $(".well_sel_data-"+language_id).val(data);
          var image_array = JSON.parse(data);
          ...
          for (var i = 0; i < image_array.length; i++) {
            ...
            var img_name = image_array[i];
            console.log("img_name", 'deleteImg(' + (i + 1) + ',' + user_id + ',"' + img_name + '")');
            htmlData += '<div class="trans_img trans_img-' + (i + 1) + '"><a href="{{ url("/public") }}/uploads/education_degree/' + img_name + '" target="_blank"><i class="fa fa-file" aria-hidden="true"></i>' + image_array[i] + '</a><div class="close_btn close_btn-' + i + '" onclick="deleteEducationImg(' + (i + 1) + ',' + user_id + ',\'' + img_name + '\','+language_id+'\,\''+name_arr+'\')" style="cursor: pointer;"><i class="fa fa-close" aria-hidden="true"></i></div></div>';
          }
    ...
    function deleteEducationImg(i, user_id, img,lang_id,language_id,name_arr) {
      $.ajax({
        type: "post",
        url: "{{ route('nurse.deleteEduEvidenceImg') }}",
        data: {
          user_id: user_id,
          img: img,
          lang_id: lang_id,
