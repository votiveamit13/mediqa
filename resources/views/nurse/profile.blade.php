...
.badge-current  { background: #EAF3DE; color: #3B6D11; }
.badge-former   { background: #F1EFE8; color: #5F5E5A; }
.badge-upskill  { background: #FAEEDA; color: #854F0B; }
              ...
              <div class="tab-pane fade" id="tab-my-jobs" role="tabpanel" aria-labelledby="tab-my-jobs" style="display: none">
                ...
                <div class="card shadow-sm border-0 p-4 mt-30">
                  ...
                  <div class="profession_summury_table">
                    <table class="table" style="width:100%;border-collapse:separate;border-spacing:0;font-size:13px">
                      <thead>
                        <tr style="background:var(--bs-light, #f8f9fa)">
                          <th style="padding:10px 14px;font-size:12px;font-weight:500;color:black;border-bottom:1px solid #dee2e6">S. No.</th>
                          <th style="padding:10px 14px;font-size:12px;font-weight:500;color:black;border-bottom:1px solid #dee2e6; width:100px">Type of nurse</th>
                          <th style="padding:10px 14px;font-size:12px;font-weight:500;color:black;border-bottom:1px solid #dee2e6">Specialty</th>
                          <th style="padding:10px 14px;font-size:12px;font-weight:500;color:black;border-bottom:1px solid #dee2e6">Status</th>
                          <th style="padding:10px 14px;font-size:12px;font-weight:500;color:black;border-bottom:1px solid #dee2e6;text-align:center">Principal</th>
                          <th style="padding:10px 14px;font-size:12px;font-weight:500;color:black;border-bottom:1px solid #dee2e6">Years of Experience</th>
                          <th style="padding:10px 14px;font-size:12px;font-weight:500;color:black;border-bottom:1px solid #dee2e6">Last Practiced</th>
                          <th style="padding:10px 14px;font-size:12px;font-weight:500;color:black;border-bottom:1px solid #dee2e6">Experience Count</th>
                          <th style="padding:10px 14px;font-size:12px;font-weight:500;color:black;border-bottom:1px solid #dee2e6">Action</th>
                        </tr>
                      </thead>
                      <tbody>
                        @if(count($profession_data) > 0)
                          ...
                          @foreach ($profession_data as $exp_data)
                            @php
                              ...
                              $spec_status     = DB::table("speciality_status")->where("status_id", $exp_data->speciality_status)->first();
                              $exp_count       = DB::table("user_experience")->where("user_id", Auth::guard('nurse_middle')->user()->id)->count();
                              $profession_id   = isset($_GET['profession_id']) ? $_GET['profession_id'] : 0;

                              $status_label = $spec_status->status_name ?? '—';
                              $badge_class = 'badge-gray';
                              if (stripos($status_label, 'current') !== false)   $badge_class = 'badge-current';
                              elseif (stripos($status_label, 'former') !== false) $badge_class = 'badge-former';
                              elseif (stripos($status_label, 'upskill') !== false || stripos($status_label, 'transition') !== false || stripos($status_label, 'training') !== false) $badge_class = 'badge-upskill';
                            @endphp

                            <tr class="profession-row profession-row{{ $exp_data->profession_id }} @if($exp_data->profession_id == $profession_id) profession_active @endif"
                                style="border-bottom:0.5px solid #dee2e6">
                              <td style="padding:12px 14px;color:#6c757d;font-size:12px">{{ $i }}</td>

                              <td style="padding:12px 14px">
                                <div style="font-weight:500;font-size:13px">{{ $nurse_data->name ?? '—' }}</div>
                              </td>

                              <td style="padding:12px 14px">
                                <div style="font-weight:500;font-size:13px">{{ $speciality_data->name ?? '—' }}</div>
                              </td>

                              <td style="padding:12px 14px">
                                @if($status_label !== '—')
                                  <span class="prof-badge {{ $badge_class }}">{{ $status_label }}</span>
                                @else
                              ...
                              <td style="padding:12px 14px;text-align:center">
                                <span class="principal-star"
                                      data-id="{{ $exp_data->profession_id }}"
                                      style="cursor:pointer;font-size:26px;color:{{ $exp_data->is_principal == 1 ? '#EF9F27' : '#adb5bd' }}">
                                  {{ $exp_data->is_principal == 1 ? '★' : '☆' }}
                                </span>
                              </td>

                              <td style="padding:12px 14px">
                                @if($exp_data->assistent_level)
                                  <span style="font-weight:500;font-size:13px">{{ $exp_data->assistent_level }} years</span>
                                @else
                                  <span style="color:#adb5bd">—</span>
                                @endif
                              </td>
                              ...
                              <td style="padding:12px 14px">
                                @php
                                  $linked_exp = DB::table("user_experience")
                                    ->where("user_id", Auth::guard('nurse_middle')->user()->id)
                                    ->whereRaw("JSON_CONTAINS(nurseType, '\"$exp_data->nurse_data\"')")
                                    ->count();
                                @endphp
                                <span style="display:inline-flex;align-items:center;gap:5px;background:#f8f9fa;border:0.5px solid #dee2e6;padding:1px 5px;border-radius:6px;font-size:12px;color:#6c757d">
                                  <i class="fas fa-briefcase" style="font-size:11px"></i>
                                  {{ $linked_exp }} {{ Str::plural('experience', $linked_exp) }}
                                </span>
                              </td>

                              <td style="padding:12px 14px">
                                <div style="display:flex;gap:6px">
                                  <a href="{{ url('/nurse/my-profile') }}?page=profession&btn=edit&profession_id={{ $exp_data->profession_id }}"
                                    style="display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:6px;border:0.5px solid #dee2e6;background:#fff;color:#6c757d;text-decoration:none"
                                    title="Edit">
                                    <i class="fas fa-pencil-alt" style="font-size:12px"></i>
                                  </a>
                                  <a style="cursor:pointer;display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:6px;border:0.5px solid #f09595;background:#fff;color:#a32d2d"
                                    data-item="{{ $exp_data->profession_id }}"
                                    data-key="{{ $exp_data->experience_id }}"
                                    data-id="{{ $exp_data->specialties }}"
                                    data-count="{{ $i }}"
                                    class="{{ $exp_data->row_status == 'complete' ? 'delete-specialty-btn' : 'delete-draft-btn' }}"
                                    title="Delete">
                      ...
                      <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                          ...
                          <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-danger delete-experience-btn" id="confirmDelete">
                              Delete all
                            </button>
                          </div>
                    ...
                    <div class="modal fade" id="deleteNoExpModal" tabindex="-1" aria-labelledby="deleteConfirmLabel" aria-hidden="true">
                      <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                          ...
                          <div class="modal-body">
                            <p id="deleteNoConfirmText" class="mb-0">Are you sure to delete this experience?</p>
                          </div>
                          ...
                          <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-danger delete-experience-btn" id="confirmDeleteNoExp">
                              Delete
                            </button>
                          </div>
                    ...
                    $('.delete-specialty-btn').on('click', function () {
                      ...
                      //alert(btn_id);
                      $('#confirmDelete').attr('data-item', btn_id);
                    ...
                    $(".delete-draft-btn").click(function(){
                      $("#deleteNoExpModal").show();

                      var profession_id = $(this).data('item');

                      $(".delete-experience-btn").attr("data-id",profession_id);
                      // //alert(profession_id);
                      // $.ajax({
                      //   type: "GET",
                    ...
                    $(".delete-experience-btn").click(function(){
                      ...
                      $.ajax({
                        type: "GET",
                        url: "{{ url('/nurse/deleteSpecialityRows') }}",
                        data: {profession_id:profession_id,experience_id:experience_id},
                        cache: false,
                        success: function(data){
                          if(data == 1){
                              $(".profession-row"+profession_id).remove();
                              $("#deleteNoExpModal").hide();
                              Swal.fire({
                                icon: 'success',
                                title: 'Success',
                                text: 'Experience rows deleted Successfully',
...
<script>
    let hasExistingPrincipal = @json($hasPrincipalSpecialty);
//     function hasPrincipalSelected() {
//     if (hasExistingPrincipal) return true;
//     return $('.principal-checkbox:checked').length > 0;
// }
function hasPrincipalSelected(currentCheckbox = null) {
    if (hasExistingPrincipal) {
        return true;
    }
    return $('.principal-checkbox').not(currentCheckbox).is(':checked');
}
</script>
    ...
    <label class="switch">
        <input type="checkbox"
               ...
               data-id="{{ $profession_single_data->specialties }}"
               name="nurseType[{{ $profession_single_data->nurse_data }}][principal][{{ $profession_single_data->specialties }}]"
               {{ $profession_single_data->is_principal == 1 ? 'checked' : '' }}>
        <span class="slider"></span>
                        ...
                        <label class="font-sm color-text-mutted mb-10">Professional Bio</label>
                         @php
                            $professionGlobal = DB::table('profession_data')
                                ...
                                ->first();
                        @endphp
                        <textarea class="form-control" rows="4" name="bio">{{ $professionGlobal->bio ?? '' }}</textarea>
                      </div>
                    ...
                    <div class="career_advancement_goals">
                      <div class="form-group col-md-12">
                        ...
                        <textarea class="form-control car_adv_goals" rows="4" name="career_advancement_goals">{{ $professionGlobal->career_advancement_goals ?? '' }}</textarea>
                      </div>
                  ...
                  <form id="educert_form" method="POST" novalidate onsubmit="return educert()">
                    ...
                    <?php
                      if(!empty($educationData)){
                        $organization_data = json_decode($educationData->general_certification);
                      ...
                      if(!empty($educationData)){    
                        if($educationData->no_certification == 1){
                          $pmembdata = array(30);
                          $p_memb_json = json_encode($pmembdata);   
                        }else{
                    ...
                    ?>
                    <div class="form-group level-drp">
                      <input type="hidden" name="prof_cert_new" class="prof_cert_new" value="<?php echo $p_memb_json; ?>">
                      ...
                      <select class="js-example-basic-multiple addAll_removeAll_btn" data-list-id="profess_cert" name="professional_certification[]" multiple="multiple" onchange="getCertData()"></select>
                    </div>
                    <span id="reqcertificate" class="reqError text-danger valley"></span>
                    <div class="subeducational_trainingdata">
                      @foreach ($p_memb_arr as $p_arr)
                        <?php
                          //print_r($o_data[$p_arr]);
                          $country_name = DB::table("professional_certificate")->where("id",$p_arr)->first();
                          $organization_list = DB::table("professional_certificate_table")->where("cert_id",$p_arr)->orderBy('name', 'ASC')->get();
                        ...
                        <div class="courses_div courses_div-{{ $p_arr }}">
                          <div class="form-group level-drp mandatory_courses_div  mandatory_tr_div_1">
                            ...
                            <ul id="well_self_care_data-{{ $p_arr }}" style="display:none;">
                              ...
                              <li data-value="{{ $org_list->professionalcert_id }}">{{ $org_list->name }}</li>
                              
                              @endforeach
                              @endif
                            </ul>
                            <select class="js-example-basic-multiple addAll_removeAll_btn" data-list-id="well_self_care_data-{{ $p_arr }}" name="well_self_care_data[]" onchange="showEducationData('ap','{{ $p_arr }}','{{ $country_name->name }}')"  multiple="multiple"></select>
                          ...
                          <div class="well_self_care_div-{{ $p_arr }}">
                            ...
                            <?php
                              ...
                              $country_name = DB::table("professional_certificate_table")->where("professionalcert_id",$p_arr1)->first();
                            ...
                            <div class="well_self_care_{{ $p_arr1 }}">
                              ...
                              <div class="row">
                                  ...
                                  <input class="form-control upload_evidence upload_evidence-{{ $p_arr1 }} well_upload_certification well_imgs_{{ $p_arr1 }} well_upload_certification-{{ $p_arr1 }}" type="file" name="" onchange="changeEducationImg({{ $user_id }},{{ $p_arr }},{{ $p_arr1 }},'general_certification')" multiple>
                                    ...
                                    <?php
                                      if(isset($oss_data['evidence_imgs'])){
                                        ...
                                        <div class="trans_img trans_img-{{ $i+1 }}">
                                          ...
                                          <div class="close_btn close_btn-' + i + '" onclick="deleteEducationImg({{ $i+1 }},{{ $user_id }},'{{ $ev_img }}',{{ $p_arr }},{{ $p_arr1 }},'general_certification')" style="cursor: pointer;"><i class="fa fa-close" aria-hidden="true"></i></div>
                                        </div>
                                        <?php
                                          $i++;
                                        ?>
            ...
            <div class="row">
                    ...
                    <div class="previous_employeers">
                      ...
                      <div class="work_exp exp_tab exp_tab-{{$i}}">
                        ...
                        <div class="form-group drp--clr nurse_exp_type nurse_exp_type-{{ $i }}">
                            <label class="form-label" for="input-1">Type of Nurse?</label>
                            @php
                              $nurse_type = (!empty($profession))?$profession->nurse_data:0;
                              $profession_specility_id = (!empty($profession))?$profession->specialties:0;
                      ...
                      <div class="work_exp exp_tab-{{ $x }}">
                        <div class="condition_set">
                          ...
                          <div class="form-group drp--clr nurse_exp_type nurse_exp_type-{{ $x }}">
                            <label class="form-label" for="input-1">Type of Nurse?</label>
                            @php
                              $nurse_type = (!empty($profession))?$profession->nurse_data:'';
                              $profession_specility_id = (!empty($profession))?$profession->specialties:'';
                              $practitioner_data = DB::table("practitioner_type")->where("id",$nurse_type)->first();
                              $mainnurse_json = (!empty($practitioner_data))?json_encode($practitioner_data->parent):'';
                              $subnurse_json = (!empty($profession))?json_encode($profession->nurse_data):'';
              ...
              <div class="tab-pane fade" id="tab-references" role="tabpanel" aria-labelledby="tab-references" style="display: none"><br>
                  ...
                  <form id="reference_form" method="POST" onsubmit="return updateReference()">
                          ...
                          @if(count($get_reference_data)>0)
                              ...
                              @foreach($get_reference_data as $referee_data)
                                  ...
                                  <div class="referee_data referee_data-{{ $i }}" data-index="{{ $i }}">
                                              ...
                                              <div class="col-md-12">
                                                  <div class="form-check">
                                                      ...
                                                      <label class="form-check-label" for="unlink-referee-{{ $i }}">
                                                          Add referee not linked to an experience
                                                      </label>
                                                  </div>
                                              </div>
                                          </div>
                                      ...
                                      @php
                                        $experience_id = isset($_GET['experience_id'])?$_GET['experience_id']:0;
                                        $profession_data = DB::table("profession_data")->where("experience_id",$referee_data->experience_id)->get();
                                      @endphp
                                      <!-- Role Section -->
                                      <div class="row role-section">
                                          @if($isLinked)
                                              <!-- For linked referees: Show info message -->
                                                  ...
                                                  @foreach($profession_data as $prof_data)
                                          <div class="col-md-6">
                                              <div class="form-group level-drp">
                                                  <label class="form-label">Type of Nurse</label>
                                                  @php
                                                    $nurse_name = DB::table("practitioner_type")->where("id",$prof_data->nurse_data)->first();
                                                    $speciality_name = DB::table("speciality")->where("id",$prof_data->specialties)->first();
                                                  @endphp
                                                  <input type="text"
                                                        class="form-control"
                                                        value="{{ $nurse_name->name }}"
                                                        disabled>

                                                  <input type="hidden"
                                                        name="nurse_type[]"
                                                        value="{{ $prof_data->nurse_data }}">
                                              </div>
                                          </div>
                                          <div class="col-md-6">
                                              <div class="form-group level-drp">
                                                  ...
                                                  <input type="text"
                                                        class="form-control"
                                                        value="{{ $speciality_name->name }}"
                                                        disabled>

                                                  <input type="hidden"
                                                        name="specialty[]"
                                                        value="{{ $prof_data->specialties }}">
                                              </div>
                                          </div>
                                          <div class="row mt-3">
                                              <div class="col-md-6">
                                                  <div class="form-group level-drp">
                                                      <label class="form-label">Employeement Start Date</label>
                                                      <input class="form-control start_date start_date-1" 
                                                            type="date" 
                                                            name="start_date[]"
                                                            onchange="startDate('1')"
                                                            value="{{ $prof_data->emp_start_date ?? '' }}" 
                                                            readonly>
                                                  </div>
                                              </div>
                                              <div class="col-md-6">
                                                  <div class="form-group level-drp">
                                                      <label class="form-label">Employeement End Date</label>
                                                      <input class="form-control end_date end_date-1" 
                                                            type="date" 
                                                            name="end_date[]"
                                                            value="{{ $prof_data->emp_end_date ?? '' }}" 
                                                            readonly>
                                                  </div>
                                              </div>
                                          </div>
                              ...
                              <div class="referee_data referee_data-1" data-index="1">
                                  ...
                                  <div class="row role-section">
                                      @if($experience)
                                          ...
                                          //   }
                                          $experience_id = isset($_GET['experience_id'])?$_GET['experience_id']:0;
                                          $profession_data = DB::table("profession_data")->where("experience_id",$experience_id)->get();
                                          ?>
                                          ...
                                          <div class="row mt-3">
                                              <div class="col-md-6">
                                                  <div class="form-group level-drp">
                                                      <label class="form-label">Referee Start Date</label>
                                                      <input class="form-control start_date start_date-1" 
                                                            type="date" 
                                                            name="start_date[]"
                                                            onchange="startDate('1')"
                                              ...
                                              <div class="col-md-6">
                                                  <div class="form-group level-drp">
                                                      <label class="form-label">Referee End Date</label>
                                                      <input class="form-control end_date end_date-1" 
                                                            type="date" 
                                                            name="end_date[]"
                      ...
                      <br>
                      <div class="add_new_certification_div mb-3 mt-3">
                          <a style="cursor: pointer;" onclick="add_another_referee()">+ Add another referee not linked to an experience</a>
                      </div>
...
<script type="text/javascript">
  ...
  if ($(".prof_cert_new").val() != "") {
    ...
    for(var i=0;i<prof_cert_new.length;i++){
      if ($(".country_org-"+prof_cert_new[i]).length > 0 && $(".country_org-"+prof_cert_new[i]).val() != "") {
        var suborg_country = JSON.parse($(".country_org-"+prof_cert_new[i]).val());
        console.log("suborg_country",suborg_country);  
        $('.js-example-basic-multiple[data-list-id="well_self_care_data-'+prof_cert_new[i]+'"]').select2().val(suborg_country).trigger('change');
      }
    }  
  }
...
$(document).on('change', '.principal-checkbox', function () {
    ...
    if (hasPrincipalSelected($cb)) {
        Swal.fire({
            title: 'Set as Primary specialty?',
            text: 'This will replace your current Primary specialty.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $('.principal-checkbox').not($cb).prop('checked', false);
            } else {
