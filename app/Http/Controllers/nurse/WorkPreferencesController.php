...
class WorkPreferencesController extends Controller{
    ...
    public function updateLocationPreferences(Request $request)
    {
        ...
        $deleted_countries = json_decode($request->deleted_country, true) ?? [];
        $multiLocationInput = $request->multiLocationInput;
        $multiDistance = $request->multiDistance;
        $international_toggle = isset($request->international_toggle)?1:0;
        $work_rights = $request->work_rights ?? [];
        ...
        if(!empty($work_preferences_data)){
            ...
            if($international_toggle == 1){
                $countries = array_values(array_unique($countries));
                $deleted_countries = array_values(array_unique($deleted_countries));
                $run = WorkPreferencesModel::where('user_id', $user_id)->update([
                    'international_relocation' => $international_toggle,
                    'countries' => json_encode($countries),
                    'deleted_countries' => json_encode($deleted_countries),
                    'work_rights' => json_encode($work_rights),
                    'pathway' => json_encode($pathway)
                ]);
            }
